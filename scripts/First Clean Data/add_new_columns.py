...
#used ai to create this dorm list for me
housing_list = [
    "Alamito Hall", "Balcones Apartments", "Bexar Hall", "Blanco Hall", "Bobcat Village Apartments", "Brogdon Hall",
    "Butler Hall", "Castro Hall", "Chautauqua Hall", "Cibolo Hall", "College Inn", "Cypress Apartments",
    "Elena Zamora O'Shea Hall", "Falls Hall", "First Five Freedom Hall", "Gaillardia Hall", "Jackson Hall",
    "Lantana Hall", "Laurel Hall", "Mesquite Hall", "Retama Hall", "San Jacinto Hall", "San Marcos Hall", "Sayers Hall",
    "Sterry Hall","Tower Hall"
]
#split the tags list to get the housting location for students
def classify_housing(tag_str):
    for dorm in housing_list:
        if dorm.lower() in tag_str:
            return dorm 
    return "Off Campus"
...
df = df.drop(columns = ["Categories", "Classification", "Location", "Cumulative GPA"])

tag_index = df.columns.get_loc("Tags")
df.insert(tag_index + 1, "HousingType", "")
df["Tags"] = df["Tags"].fillna("").str.lower()
df["Housing Type"] = df["Tags"].apply(classify_housing)

#This is used so that the last 3 rows that contain "unique students" is deleted
